...
export const useSocketFriendListener = (socket) => {
  ...
  const handleReceiveFriendRequest = useCallback(
    (data) => {
      ...
      const existingSuggestedUsers = queryClient.getQueryData(["suggestedUsers"]);

      if (existingSuggestedUsers) {
        // Remove the requester from suggestedUsers cache
      ...
      queryClient.setQueryData(["friendRequests"], (oldData) => {
        ...
        // Flag to track if we found an existing entry
        let found = false;
